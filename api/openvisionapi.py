...
class OpenVisionAPI:
	...
	def __init__(self, ip='127.0.0.1', port=4242, logfile='default.tsv', \
		...
		"""The connection class communicates to the GazePoint
		...
		ip	-	The IP address of the computer that is running the
				 server. This will usually be the localhost at
				127.0.0.1. Type: str. Default = '127.0.0.1'
		
		port	-	The port number that the  server is on; usually
		...
		if self._debug:
			...
			self._debuglog.write("PYTHON DEBUG LOG %s\n" % (dt))
			self._debugcounter = zero
		...
		self._logthread = Thread( \
			...
			name='PyVision_OpenVisionConnection_logging', \
			args=[])
		...
		self._inthread = Thread( \
			...
			name='PyVision_OpenVisionConnection_incoming', \
			args=[])
		...
		self._outthread = Thread( \
			...
			name='PyVision_OpenVisionConnection_outgoing', \
			args=[])
	...
	def _debug_print(self, msg):
		...
		if self._debug:
			...
			if self._debugcounter % self._debugconsolidatefreq == zero:
				self._debuglog.flush()
				os.fsync(self._debuglog.fileno())
			self._debugcounter += one
	...
	def _process_logging(self):
		...
		while not self._log_ready_for_closing.is_set():
			...
			if self._logcounter % self._log_consolidation_freq == zero:
				self._log_consolidation()

			# Increment the counter.
			self._logcounter += one
	...
	def _process_incoming(self):
		...
		while self._connected.is_set():
			...
			# Get new messages from the OpenVision Server.
			timeout = False
			try:
	...
	def _process_outgoing(self):
		...
		while not self._sock_ready_for_closing.is_set():
			...
			# accessing it.
			self._socklock.acquire()
			# Send the command to the OpenVision Server.
			t = time.time()
			self._sock.send(msg)
			# Unlock the socket again.
			self._socklock.release()
	...
	def close(self):
		...
		# Close the socket connection to the OpenVision server.
	...
	def calibrate_start(self, state):
		...
		if state:
			self._current_calibration_point = zero
		else:
			self._current_calibration_point = None
